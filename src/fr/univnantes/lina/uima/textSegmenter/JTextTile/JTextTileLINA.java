 ...
 * 
 * Classe modifiée par Nicolas Hernandez le 08/03/11
 * Principalement changement des signatures de certaines méthodes au niveau de 
 * la portée (les tâches principales sont exécutées par le main et le 
 * constructeur JTextTileLINA ; ce sont les méthodes appelées qui ont été 
 * rendues accessibles de l'extérieure de la classe), initialisation différente
 * de certaines variables, et mise en commentaire du main
 * 
 * classe originale: C99.java
 * @author: Freddy Choi
 */
public class JTextTileLINA {
	/* Program parameters */
	protected int w = 100; // Size of the sliding window
	protected int s = 10; // Step size
		
	/* Data sets */
	// Changed by N Hernandez: before initialization with constructor ( no Stopword() constructor with such signature  available), after to null value
	// before //protected Sentences C =  new RawText(); // A collection for segmentation
	protected Sentences C = null ;  
 ...
 * 
 * Modified by Nicolas Hernandez 110308 :  commented
 */
	
/*	
public JTextTileLINA(Sentences c, Stopword s, int w, int step) {
	C = c;
	S = s;
	this.w = w;
	this.s = step;
 ...
 * 
 * Modified by Nicolas Hernandez 110308 :  private -> protected
 */
 ...
 * 
 * Modified by Nicolas Hernandez 110308 :  commented (embedded comments adapted)
 */
/*
public static void main(String[] args) {
	...
	try {
		...
		// Lets boogie 
		JTextTileLINA t = new JTextTileLINA(c, s, window, step);		// Initialise text tiling algorithm with collection
		JTextTileLINA.genOutput(c, t.boundaries);				// Generate segmented output
		System.exit(0);
	}
	catch (Exception e) {
		Debugx.handle(e);
